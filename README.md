...
### app_groups.json（应用分组覆盖层，运行期生成）

位于 `data_root` 下，由仪表盘「分组」视图写入（原子写 + 5s TTL 缓存），也可手工编辑：

```json
{ "exe_groups": { "steam.exe": "我的分组" }, "custom_categories": ["我的分组"] }
```

`exe_groups` 的 key 为小写 exe 名，分类时精确匹配且优先级最高；`custom_categories`
为自定义分组名列表。缺失或损坏时自动按空覆盖层处理（不影响自动分类）。
