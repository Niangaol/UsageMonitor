# -*- coding: utf-8 -*-
"""browser_history.py — 浏览器 URL 级历史解析（Phase 3，Chromium 系：Chrome/Edge）。

功能：
- 读取浏览器 History SQLite（urls/visits 表）得到某天访问的 URL 明细（时间/域名/标题/分类）；
...
def find_history_dbs(config: dict | None = None) -> list[dict]:
    """发现 Chromium 系浏览器的 History 数据库。

    返回 [{"browser": "chrome", "profile": "Default", "db": "绝对路径"}]。
    config.browser_history 可覆盖 user_data 路径（None = 自动探测）。
    """
    if config is None:
        config = classifier.load_config()
    specs = config.get("browser_history", {})
    if not isinstance(specs, dict):
        specs = {"chrome": {"user_data": None}, "edge": {"user_data": None}}
...
# 提取与分类
# ---------------------------------------------------------------------------
def extract_visits(db_copy_path: str, day_str: str, config: dict | None = None) -> list[dict]:
    """从 History 数据库副本提取某天的访问记录（时间升序）。
    ...
    try:
        ...
        for visit_time, visit_duration, url, title in cur:
            ...
            visits.append({
                "time": datetime.datetime.fromtimestamp(overlap_start).strftime("%Y-%m-%dT%H:%M:%S"),
                "url": url_disp,
                "title": title_disp,
                "domain": domain or "-",
                "category": category,
    ...
    with tempfile.TemporaryDirectory(prefix="usagemon_hist_") as tmpdir:
        for item in dbs:
            ...
            if copy is None:
                ...
                continue
            try:
                vs = extract_visits(copy, day_str, config)
                for v in vs:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list_browsers:
        dbs = find_history_dbs(config)
        if not dbs:
            print("未发现 Chrome/Edge 历史数据库（可配置 browser_history.user_data 路径）")
            return 0
        for item in dbs:
